...
from loader.entity import extract_entities, convert_to_sub_words
...
def entity2network(sentence_data, words, params, tokenizer):
    ...
    tags_arg = sentence_data['tags_arg']
    tags_arg_terms = sentence_data['tags_arg_terms']

    sw_sentence, sw_sentence_arg, sub_to_word, subwords, valid_starts = convert_to_sub_words(words,
                                                                            list(map(list, zip(tags[0]))),
                                                                            list(map(list, zip(tags_terms[0]))),
                                                                            tags_arg, tags_arg_terms,
                                                                            tokenizer=tokenizer)
    ...
    tagsIDs = list(map(list, zip(tagsIDs[0])))
    ...
    return readable_e, idxs, ents, toks2, etypes2ids, entities, sw_sentence, sw_sentence_arg, sub_to_word, subwords, valid_starts, tagsIDs, terms
